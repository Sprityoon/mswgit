...
> **2026-07-04 확정 구현** (아래 표의 이름은 원안 — 실제로는 기존 엔티티 이름 유지): 타일은 **9방향(3x3)만** 사용,
> `wall.tileset` 단일 시트 (`FullGrass` 베이스 / `Soil`+8방향 길 / `TerraceTop` 9종 / `CliffFace` 6종 / `Big Wall` 충돌).
...
> `RectTileMap4`(SL3 Big Wall 밴드) / `RectTileMap5`계열(SL4 경계 테라스 비주얼) / MapLayer5 = 엔티티 전용.
> 로직: `ResourceSpawner:ComputeAutotileName(prefix, mask, x, y)` 9방향 공용 + `AutotileSoilLayer` 런타임 보정.
> 페인팅: `scripts/build_maps.cjs` (map01·town·template_field·template_boss 일괄, 멱등).
> ⚠️ 생성기 제약(실측): **레이어 엔티티 리네임/삭제 금지** — Maker refresh 증분 적용이 깨짐(LEA-3054). 이름 유지 + 신규 생성만 안전.
