...
| Phase | 내용 | 완료 기준 |
|:---:|------|-----------|
| S1 | 패키지 통합 (Core만) + 처치/채집/제작/제련 ActionEvent 훅 + 조건 클래스 | 업적 진행도가 플레이로 카운트됨 |
| S2 | SkillDataSet 확장 + SP 지급 + 서버 해금/레벨업 + Persistence 필드 | CSV로 정의한 스킬이 조건 충족 시 SP로 해금·강화됨 |
| S3 | SkillTreePopup UI + SkillBar 연동 | 트리에서 해금→장착→사용 전체 루프 |
| S4 | 스킬·업적 밸런싱 CSV 채우기 + 검증 | 초기 트리(예: 전투/채집 2계열 × 4~6스킬) 플레이 가능 |
...
1. 하드코딩 8곳 → 공용 @Logic 상수(`_RenderLayers.EntityLayer` = "MapLayer5")로 일원화
2. 런타임 스폰 시 `OrderInLayer` 명시(≥2) — 코드 생성 스프라이트는 모델 배치 기본표(타일1/몬스터2/아바타4)가 보장되지 않음
