...
| Phase | 내용 | 완료 기준 | 상태 (2026-07-03) |
|:---:|------|-----------|------|
| S1 | 패키지 통합 (Core만) + 처치/채집/제작/제련 ActionEvent 훅 + 조건 클래스 | 업적 진행도가 플레이로 카운트됨 | 코드 완료 — **플레이 검증만 남음 (Maker MCP 필요)** |
| S2 | SkillDataSet 확장 + SP 지급 + 서버 해금/레벨업 + Persistence 필드 | CSV로 정의한 스킬이 조건 충족 시 SP로 해금·강화됨 | ✅ 구현 완료 (`ServerRequestSkillLevelUp`, `SkillLevelsJson`/`SP` 저장, `SPPerLevelUp`=3) |
| S3 | SkillTreePopup UI + SkillBar 연동 | 트리에서 해금→장착→사용 전체 루프 | ✅ 구현 완료 (K키 토글, `Node_<row>_<col>` 범용 4x3 슬롯, 스킬바 잠금 표시) |
| S4 | 스킬·업적 밸런싱 CSV 채우기 + 검증 | 초기 트리(예: 전투/채집 2계열 × 4~6스킬) 플레이 가능 | 데이터 완료 (전투4+채집 패시브2, 업적 1005 추가, `PassiveStat`/`PassiveValuePerLevel` 컬럼) — **플레이 검증 남음** |
...
3. 모델 7종(Slime/Boar/HornMushroom/SlimeKing/Merchant/Building_House/Building_ResearchLab) SortingLayer 값 MapLayer0→MapLayer5 (ModelBuilder 일괄)
4. 플레이어 아바타: 플레이 테스트에서 테라스 타일 아래로 숨는지 확인 → 숨으면 DefaultPlayer에 오버라이드 추가
5. 레이어 엔티티 이름 정규화 (map01은 6레이어이나 RectTileMap_1=MapLayer4 등 이름 뒤죽박죽)
