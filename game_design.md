...
- [ ] **19-A 아이템 아이콘 교체 및 일치화** (T72): 아이콘과 실제 모델 모양의 불일치 전수 조사 및 교체. 모든 UI 표시 정비.
- [ ] **19-B 마을 광장 리스킨 및 배치** (T73): 대장간, 분수, 우물 신규 `.model` 추가 및 심미적 재배치. (상점 제외)
- [ ] **19-C 주거구역 주택 배치** (T74): 주거 건물(버섯집 등) 5동 확보 및 배치.
- [ ] **19-D 상점거리 소품 및 생활감** (T75): 벤치, 가로등, 울타리 등 데코 14종 다중 배치.
- [ ] **19-E 마을 랜드마크 건물** (T76): 스카이라인 형성용 대형 건물(여관, 헛간, 시계탑) 배치.
- [ ] **19-F 비전투 NPC 및 생물** (T77): `DialogDataSet` 연동된 주민 NPC 4인 및 고양이 신규 리소스 확보 및 배치.
- [ ] **19-G 필드 및 바이옴 변주** (T78): 다양한 나무, 바위 추가로 사냥터 다변화. 낚시터 모델 리스킨 적용.
