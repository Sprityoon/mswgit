...
- ⛔ **비범위 (⚖️ 2026-07-11 확정)**: 영지 방문·랭킹(후속 Phase 후보) / 계절 테마(아트 확정 후) / **허기 시스템(페널티형 — 아늑한 생활 톤과 충돌, 기각)** / 가축 번식·펫 전투·날씨-몬스터 연동 / 낚시 게이지 미니게임 고도화.

### Phase 16: 스킬 메이플 원작화 & 전직 로드맵 (⚖️ 2026-07-14 보스 지시 — 설계: [docs/design/skill-tree-plan.md](./docs/design/skill-tree-plan.md) §6)
> 스킬트리 인프라(SP·업적/레벨 게이트·K 트리 팝업 — skill-tree-plan S1~S4, 2026-07-05 검증)는 완성 상태. 이번 축 = **해금 흐름을 실제로 밟게 만들고**(QWER 선장착 기본값 제거) **원작 메이플 스킬 리소스로 스킨**. 구현 티켓 = handoff §3 **T45→T46 (배치 F)**. 배치 C(15-E 목장·15-H 펫)는 배치 F 완료 후(`PlayerController`/`PersistenceManager` 소유 겹침).
...
- [ ] **16-A′ 스킬트리 UX 보강** (T47 — 제작자 Play 피드백 2026-07-14): ① 노드 클릭=SP 즉시 투자 결함 분리 — 클릭은 선택/상세만, 투자는 [레벨업] 버튼으로 ② 스킬트리 여는 HUD 버튼 신설(⚖️ 보스 확정 — 온스크린 시전 버튼/모바일은 범위 밖, 후속 후보 key-binding-package).
- ⏳ **16-C 직업/전직 (예약 — 티켓 미발행)**: `JobId` 컬럼 예약(공란=초보자 공용) / 직업 선택·전직 퀘스트(quest-achievement 패키지 재사용)·직업별 트리 탭·상위 스킬군. 16-A/B 안착 후 기획 확정.
