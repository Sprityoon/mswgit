...
- **맵 구성 = 고정 레이아웃 풀 + 주기 로테이션 (Fixed Pool + Periodic Rotation)**: 매 방문마다 무작위로 다시 굴리지 않는다. 대신 **미리 선정한 소수의 고정 레이아웃 풀**을 두고, **서버 시간 기준 약 3시간(`RotationPeriod`, 데이터화)마다 활성 레이아웃이 바뀐다**. 한 주기(window) 동안은 모두에게 **동일한 고정 지형**이라 "오늘의 사냥터" 느낌을 준다.
  - **레이아웃 풀 = 큐레이션된 시드 세트 (기본 구현)**: 멀티바이옴 생성기(`ResourceSpawner`)에 **손으로 고른 시드 몇 개**를 넣어 결정론적 고정 지형을 만든다. 활성 시드 = `curatedSeeds[ floor(serverTime / RotationPeriod) % N ]`. (향후 손으로 디자인한 `.map` 변종을 풀에 섞는 확장도 가능 — 로테이션 메커니즘은 동일.)
  - **주기 경계 동작**: 3시간 경계에서 공용 사냥터를 새 시드로 **재생성**(자원/몬스터 리프레시). 사냥터에 있던 유저는 "지형이 새로 갈렸다" 식의 리프레시를 본다(허용 가능한 연출).
  - **손으로 박는 고정 픽스처**: 복귀 포탈, NPC, 보스 스폰 지점 등은 `.map`에 직접 배치한다(시드 로테이션과 무관하게 상주). 지형 생성기와 맵 픽스처는 레이어가 달라 충돌하지 않는다.
- **초기 사냥터**: 처음부터 **쉽게 드나들 수 있어야** 한다(영지 포탈 목록에서 바로 진입).
- **해금형 사냥터**: 이후 사냥터는 **퀘스트** 또는 **일정 레벨** 도달로 해금한다. (이번 단계 범위 밖 — 목록+이동 우선.)
- **특수 던전 (디아블로2 카우방식)**: **특수 조건을 만족**하거나 **전용 포탈 아이템을 구매/획득**해 보유 중일 때만 목록에 노출/입장한다(D2 카우방/우버 포탈식). (이번 단계 범위 밖.)
- ⚠️ 이 전환으로 광석(Copper/Iron)·Big Stone이 개인 영지에서 사라지므로, **금속 테크(§4 T3/T4)는 사냥터 출시 후 본격 가동**된다(의도된 의존성).
...
- **데이터 주도**: 목적지(맵/표시명/도착좌표/해금조건/필요아이템)는 코드 분기 대신 `PortalDestinationDataSet`로 관리해, 사냥터·특수던전 추가가 **행 추가만으로** 반영되게 한다.
...
### Phase 12: 사냥터 진입 — 목록 포탈 + 첫 사냥터 (진행 중) — §2.2 ③ / 포탈 시스템
> **방향**: 영지에서 들어낸 전투·광석·드롭이 살 사냥터를 신설하고, 디아블로2 웨이포인트식 목록 포탈로 진입한다. 이번 단계 범위는 **목록 + 이동까지** (해금 게이팅·특수던전은 다음 단계).

- **12-A 마을→영지 복귀 단순화 (작업 단위)**:
  - [ ] `PlayerController`의 town 분기(WarpPopup 오픈) 제거 → 마을 포탈 상호작용 시 `ServerRequestWarp()`로 본인 영지 직행(town `PortalToHome`는 `TargetMapName="Home"` 이미 설정됨). 유저 이름 입력 방문(`UIWarpController`/WarpPopup/`ServerRequestVisitHome`)은 비활성 보존(향후 재설계). 빌드 로그 검증 후 커밋.
- **12-B 첫 사냥터 공용 맵 신설 — 고정 풀 + 3h 로테이션 (작업 단위)**:
  - [ ] **per-map 생성 설정 리팩터**: `ResourceSpawner`가 맵별로 `seed`/`forceBiomeId`를 다르게 쓰도록 분리(현재 단일 `PrngSeed`/`ForceBiomeId` 싱글턴 → `GetMapSeed(mapName)`/`GetMapForceBiomeId(mapName)` + per-map 테이블, 기본값은 영지=green_island/기본시드 그대로 유지). `GetBiomeAt`/`IsGrassAt`에 mapName 전달.
  - [ ] 공용 정적 사냥터 `.map`(예: `hunt01`) 생성(RectTile, town 부류 상주 공용 맵). `forceBiomeId=""`로 멀티바이옴 적용 + `MonsterSpawner` 처리 대상에 포함. 손배치 복귀 포탈(→ town) 픽스처.
  - [ ] **3h 시드 로테이션 매니저**: 큐레이션 시드 풀 + `RotationPeriod`(기본 10800초)로 활성 시드 산출, 경계에서 hunt01 재생성(자원/몬스터 리프레시). 빌드 로그 검증 후 커밋.
- **12-C 목적지 데이터셋 + 목록 포탈 UI (작업 단위)**:
  ...
  - [ ] 기존 영지 `Portal` 가구를 목록형으로 확장 — F/상호작용 시 목적지 목록 팝업(스크롤 리스트) → 선택 시 워프(`ServerRequestWarp` 계열 재사용/확장). 이번 단계는 해금 무시(전부 노출)·이동만. 빌드 로그 검증 후 커밋.
- **후속 단위 (Phase 12 이후)**:
  - [ ] 사냥터 해금(레벨/퀘스트) 게이팅 — 목록 잠금 표시/차단.
  - [ ] 카우방식 특수 던전 — 전용 포탈 아이템 구매/획득 시 목록 노출·소비 입장.
  - [ ] 사냥터 추가(다중) + 영지↔마을↔사냥터 순환 동선 정교화.
