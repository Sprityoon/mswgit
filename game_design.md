...
- **초기 사냥터**: 처음부터 **쉽게 드나들 수 있어야** 한다(마을/영지에서 바로 진입).
- **해금형 사냥터**: 이후 사냥터는 **퀘스트** 또는 **일정 레벨** 도달로 해금한다.
- **특수 던전 (디아블로2 카우방식)**: **특수 조건을 만족**하거나 **특수 아이템으로만 열 수 있는** 던전 개념을 구상한다.
- **구현 메모**: 기존 개인 영지에 쓰이던 **멀티바이옴 매크로 그리드 생성 로직**(desert/rocky/snowfield/earth_field + 변종 몬스터 스폰)을 **사냥터 맵으로 이관·재사용**하는 방향. ⚠️ 이 전환으로 광석(Copper/Iron)·Big Stone이 개인 영지에서 사라지므로, **금속 테크(§4 T3/T4)는 사냥터 출시 전까지 사실상 보류**된다(의도된 의존성).
...
- **하이브리드 맵 구조 (동적 개인 영지 + 공용 마을 + 공용 사냥터)**:
  ...
  - **내 집 / 개인 영지 (Home_UserID - 1인 전용 공간)**: 플레이어 진입 시 서버에서 map01 템플릿을 복제하여 동적으로 생성하며, 퇴장 시 메모리에서 삭제됩니다. **green_island 단일 바이옴 + 반경 30(61×61) + 느린 자원 생성 + 전투 몹 없음**으로 구성됩니다(§2.2 ①). 절차 지형은 초기 캔버스이고 **플레이어 설치 타일/가구가 항상 우선**입니다.
...
- **후속 단위 (이번 세션 범위 밖, 설계만 확정)**:
  ...
  - [ ] 사냥터 맵 신설 + 멀티바이옴/전투 로직 이관 + 초기 사냥터 진입 동선.
  - [ ] 사냥터 해금(레벨/퀘스트) + 카우방식 특수 던전(특수 조건/아이템 입장).
