...
- **맵 구성 = 하이브리드 (직접 디자인 + 절차 생성 공존)**: 공용 정적 `.map`을 베이스로,
  - **절차 지형/자원/몬스터**: 기존 멀티바이옴 생성 로직(`ResourceSpawner` ForceBiomeId="" 경로 + `MonsterSpawner`)을 **이 맵에도 돌려** 바이옴 지형·자원·변종 몬스터를 입힌다.
  - **손으로 박는 고정 픽스처**: 복귀 포탈, NPC, 보스 스폰 지점 등은 `.map`에 직접 배치한다. (지형 생성기와 맵 픽스처는 레이어가 달라 충돌하지 않는다.)
...
- **12-B 첫 사냥터 공용 맵 신설 (작업 단위)**:
  - [ ] 공용 정적 사냥터 `.map`(예: `hunt01`) 생성(RectTile, town 부류의 상주 공용 맵). 절차 멀티바이옴 생성을 이 맵에도 적용(`ResourceSpawner` ForceBiomeId="" 경로 + `MonsterSpawner` 처리 대상에 포함). 손배치 복귀 포탈(→ town) 픽스처 배치. 초기 사냥터는 진입 난이도 낮게. 빌드 로그 검증 후 커밋.
