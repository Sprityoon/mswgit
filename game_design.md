...
### Phase 12: 사냥터 진입 — 목록 포탈 + 단계적 사냥터 (진행 중) — §2.2 ③ / 포탈 시스템
> **방향**: 영지에서 들어낸 전투·광석·드롭이 살 사냥터를 신설하고, 디아블로2 웨이포인트식 목록 포탈로 진입한다. 사냥터는 **여러 구역이 포탈로 연결된 단계 구조**(초기 사냥터 → 연결 포탈 → 보스)이며, 새 구역에 처음 도달해 포탈을 열면 **영지 목록에 해금**된다(유저별 영속). 난이도는 **장비 티어 게이트**(T1 필드=돌·나무, T1 보스=구리 빌드; 구리 재료는 T1 필드 드롭). **MVP는 목록+이동까지**(보스/해금/로테이션은 단계적 확장).

- **12-A 마을→영지 복귀 단순화 (✅ 완료, `3ffd9ea`)**: `PlayerController`의 town 분기(WarpPopup) 제거 → 마을 포탈 상호작용 시 본인 영지 직행. 이름 입력 방문 동선 비활성 보존.
- **12-B 첫 사냥터 공용 맵 신설 (진행 중)**:
...
- **후속 단위 (Phase 12 이후 — 단계적 사냥터 본 구조)**:
  - [x] **웨이포인트 해금 (완료)**: 영지 귀환 포탈을 타는 시점에 현재 사냥터 구역의 웨이포인트 해금 데이터를 서버 단에서 자동 잠금해제 및 UserDataStorage 영속화 연동.
  - [x] **사냥터 체인 (완료)**: 사냥터 2구역(hunt02) 신설 및 hunt01-hunt02 간의 양방향 연결 포탈(하얀색)을 통한 구역 이동 구현.
...
- **12-D 영지 포탈 등급화 및 연동 고도화 (⚠️ 진행 중 — 유저 테스트 및 검증 대기)**:
  - [x] **영지 포탈 종류 세분화**: 영지 포탈 가구에 `DestinationGroup`(목적지 그룹) 및 `PortalColor` 속성을 추가하여, 배치/설치 시 클라이언트에서 고유 색상 틴트(하늘색, 초록색 등)를 실시간 동적으로 칠하도록 구현.
  - [x] **가구 철거/재배치 복원**: 포탈 가구를 가방에 넣었다가 다시 배치해도 고유 기능과 틴트 색상이 보존되도록 저장/복원 스키마 고도화.
  - [x] **초보 사냥터 포탈 기본 제공**: 신규 영지 맵 최초 생성 시, 마을행 포탈(하얀색) 옆에 초보 사냥터 팝업을 여는 사냥터 포탈(하늘색)이 기본 배치되도록 설정.
  - [x] **그룹 필터링 및 마을 즉시 이동**: 마을 포탈(`Group="town"`)은 선택 팝업 없이 즉시 마을로 이동하고, 사냥터 포탈은 지정된 `Group` 필터(예: `"rookie"`)에 해당하는 사냥터 묶음 목록만 Warp UI 슬롯에 선별 노출되도록 구현.
  - [x] **사냥터 복귀 포탈 차별화**: 사냥터 내 영지 복귀 포탈은 파란색(하늘색 틴트)으로 자동 스폰되며, 사냥터 간 연결 포탈은 기본 하얀색으로 스폰되어 직관적 구별 가능.
