...
1. **표준 게임 시스템** (인벤토리/상점/랭킹/퀘스트/우편/도감/키바인딩 등) → `msw-packages` 스킬의 공식 패키지 카탈로그 확인. 일치 패키지가 있으면 통합을 제안하고, 백지 구현은 카탈로그에 없음을 확인한 뒤에만. README 상세·프로젝트 적합성은 로컬 위키 미러(`docs/wiki/mswpackages/INDEX.md` — `msw-wiki` 스킬)를 GitHub fetch보다 우선.
...
- **프로젝트 스킬**: `msw-conductor`(지휘자 운영) / `msw-worker`(T티켓 구현) / `msw-checkpoint`(문서 동기화 + git 커밋·푸시) / `msw-wiki`(로컬 위키 `docs/wiki/` — MSWPackages 미러 + RoguelikeWorld 예제) / `image-to-pixel`(이미지→픽셀 아트).
- **벤더 `msw-planning` 스킬 주의**: "다음 작업/이어서 진행" 트리거가 겹치지만, **이 프로젝트에서 그 요청은 T티켓 큐(`msw-worker`/`msw-conductor`) 소관**이다. msw-planning은 별도 신규 게임·마일스톤 GDD 기획에만 사용.
